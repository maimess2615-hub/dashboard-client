...
#Intrpretation
if cluster == 0:
    interpretation = "Client a faible valeur"
    reco = "Promotion et acquisition"
elif cluster == 1:
    interpretation = "Client moyen"
    reco = "Fidelisation et offres ciblees"
else:
    interpretation = "Client premium"
    reco = "offres VIP et relation client personnalisee"
...
profil = f"Client avec frequece {frequence}, depenses {depenses}, anciennete {anciennete} ans.Segment : {interpretation}.Revenu estime : {prediction[0]:,.0f}"
st.write(profil)

#Storytelling automatique
st.subheader("📖Storytelling")
st.write(f"Ce client appartient a un segmet {interpretation.lower()}, avec un potentiel de revenu estime a {prediction[0]:,.0f}. Il est recommande d'appliquer une strategie : {reco.lower()}.")
